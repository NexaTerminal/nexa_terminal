...
const CyberQuestionnaire = () => {
  ...
  const fetchIndustries = async () => {
    try {
      const response = await api.get('/chc/industries');
      if (response && response.success) {
        ...
        setIndustries([
          ...
          { id: 'manufacturing', name: 'Производство / Фабрика', description: 'Производствени компании, фабрики, индустрија' },
          { id: 'retail', name: 'Трговија на мало / големо', description: 'Продавници, дистрибуција, е-трговија' },
          ...
          { id: 'startup', name: 'Стартап / SaaS / ИТ', description: 'Технолошки стартапи, софтверски компании' },
  ...
  const fetchQuestions = async (industry) => {
    try {
      setLoading(true);
      const response = await api.get(`/chc/questions?industry=${industry}`);
      if (response && response.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await handleCreditOperation(
        async () => api.post('/chc/evaluate', {
          answers: answers,
          industry: selectedIndustry
        }),
        'сајбер безбедносен преглед',
        1
      );
      ...
      if (response && response.success) {
        ...
        navigate(`/terminal/cyber-screening/report/${response.data._id}`);
      }
    } catch (err) {
          ...
          <main className={styles["dashboard-main"]}>
            <div className={styles["questionnaire-container"]}>
              <div className={styles["questionnaire-header"]}>
                <h1>Сајбер безбедносен преглед</h1>
              </div>
          ...
          <main className={styles["dashboard-main"]}>
            <div className={styles["questionnaire-container"]}>
              <div className={styles["questionnaire-header"]}>
                <h1>Сајбер безбедносен преглед</h1>
                <p>
                  Добијте објективна проценка на сајбер безбедноста на вашата компанија и дознајте каде имате простор за подобрување.
      ...
      <div className={styles["dashboard-layout"]}>
        ...
        <main className={styles["dashboard-main"]}>
          <div className={styles["questionnaire-container"]}>
            ...
            <div className={styles["questionnaire-header"]}>
              <h1>Сајбер безбедносен преглед</h1>
              <p>Одговорете на прашањата за да добиете персонализирана проценка на сајбер безбедноста на вашата компанија.</p>
            </div>
                  ...
                  <div key={question.id} className={styles["question-card"]}>
                    <div className={styles["question-number"]}>
                      ...
                      {question.isHighlyRelevant && (
                        <span style={{ marginLeft: '8px', color: '#1E4DB7', fontSize: '0.75rem' }}>
...
export default CyberQuestionnaire;
