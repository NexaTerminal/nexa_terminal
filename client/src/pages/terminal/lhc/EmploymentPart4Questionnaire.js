...
const EmploymentPart4Questionnaire = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await api.get('/lhc/employment-part4/questions');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await handleCreditOperation(
        async () => api.post('/lhc/employment-part4/evaluate', {
          answers: answers,
          companySize: companySize
        }),
        'правен здравствен преглед - посебна заштита',
        1
      );
      ...
      if (response && response.success) {
        ...
        navigate(`/terminal/legal-screening/employment-part4/report/${response.data._id}`);
      }
    } catch (err) {
          ...
          <main className={styles["dashboard-main"]}>
            <div className={styles["questionnaire-container"]}>
              <div className={styles["questionnaire-header"]}>
                <h1>Работни односи: Дел 4 - Посебна заштита</h1>
      ...
      <div className={styles["dashboard-layout"]}>
        ...
        <main className={styles["dashboard-main"]}>
          <div className={styles["questionnaire-container"]}>
            <div className={styles["questionnaire-header"]}>
              <h1>Работни односи: Дел 4 - Посебна заштита</h1>
              <p>Овој дел опфаќа прашања за заштита на бремени работнички, родители и лица со намалена работна способност.</p>
            </div>
