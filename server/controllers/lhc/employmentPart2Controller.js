...
const { questions, sanctions, gradeConfig, categoryNames, ANSWER_TYPES, SANCTION_LEVELS } = require('../../data/lhc/employmentPart2Questions');
...
class ComplianceEvaluator {
  ...
  getGradeDescription(grade) {
    const descriptions = {
      'Перфектна усогласеност': `Кај ${this.companyName} постои перфектна усогласеност во делот на заштита и престанок на работен однос.`,
      'Одлична усогласеност': `Кај ${this.companyName} постои одлична усогласеност во делот на заштита и престанок на работен однос.`,
      'Задоволителна усогласеност': `Кај ${this.companyName} постои задоволителна усогласеност во делот на заштита и престанок на работен однос.`,
      'Определена усогласеност': `Кај ${this.companyName} постои определена усогласеност во делот на заштита и престанок на работен однос.`,
      'Делумна усогласеност': `Кај ${this.companyName} постои делумна усогласеност во делот на заштита и престанок на работен однос.`,
      'Ниска усогласеност': `Кај ${this.companyName} постои ниска усогласеност во делот на заштита и престанок на работен однос.`,
      'Исклучително ниска усогласеност': `Кај ${this.companyName} постои исклучително ниска усогласеност во делот на заштита и престанок на работен однос.`
    };
...
async function evaluateCompliance(req, res) {
  try {
    ...
    const assessment = {
      userId,
      category: 'employment_part2',
      categoryTitle: 'Работни односи: Заштита и престанок',
      answers,
      companySize,
      ...report,
      createdAt: new Date()
...
async function getAssessmentHistory(req, res) {
  try {
    ...
    const assessments = await db
      ...
      .find({ userId: req.user._id, category: 'employment_part2' })
      .sort({ createdAt: -1 })
      .limit(10)
      .toArray();
