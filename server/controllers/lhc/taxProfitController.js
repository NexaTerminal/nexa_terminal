// LHC controller: Данок на добивка (tax_profit) — под-модул 1 од „Даночна усогласеност".
// Uses the shared profiling-aware scoring engine (lhcScoring.js).

const questions = require('../../data/lhc/taxProfitQuestionsComplete');
const { scoreModule } = require('./lhcScoring');

const CATEGORY_TITLES = {
  tp_expenses: 'Признати и непризнати расходи',
  tp_employee_limits: 'Надоместоци и лимити по вработен',
  tp_writeoffs: 'Отписи, кусоци и амортизација',
  tp_regime: 'Даночен режим, биланс и аконтации',
  tp_transfer_pricing: 'Трансферни цени и поврзани лица',
  tp_controls: 'Даночна контрола'
};
const CATEGORY_ORDER = [
  'tp_expenses', 'tp_employee_limits', 'tp_writeoffs',
  'tp_regime', 'tp_transfer_pricing', 'tp_controls'
];

const DISCLAIMER = 'Овој извештај е индикативна самопроценка на усогласеноста со Законот за данокот на добивка. Не претставува даночен ниту правен совет, ниту задолжителни инструкции, ниту официјална оцена на Управата за јавни приходи (УЈП) — препораките и чекорите се само насоки. Даночните стапки и лимити се менуваат, а дел од обврските важат само за одредени режими или дејности. За да ја потврдите вашата усогласеност, консултирајте овластен сметководител/даночен советник, адвокат и УЈП.';

// Profit-tax fines are scaled by trader size; a plain-language hint is enough here.
const SANCTION_HINT = 'Можна прекршочна глоба степенувана по големина на трговецот — правно лице од ~600 € (микро) до 20.000 € (голем) за неправилно утврдена основа/непризнати расходи, плус доод на данок и камата.';

function taxProfitBandDescription(band, companyName, criticalCount) {
  const map = {
    4: `Кај ${companyName} постои висока усогласеност со обврските за данокот на добивка.`,
    3: `Кај ${companyName} постои задоволителна усогласеност, но неколку ставки треба да се доуредат.`,
    2: `Кај ${companyName} постои делумна усогласеност. Постојат значителни ризици од непризнати расходи или погрешен режим.`,
...
function evaluateTaxProfit(answers, companyName) {
  const report = scoreModule({
    ...
    config: {
      ...
      describeBand: taxProfitBandDescription
    }
  });
...
async function getQuestions(req, res) {
  try {
    ...
    console.error('Error fetching tax profit questions:', error);
...
async function evaluateCompliance(req, res) {
  try {
    ...
    const report = evaluateTaxProfit(answers, companyName);
    ...
    console.error('Error evaluating tax profit compliance:', error);
...
async function getAssessmentHistory(req, res) {
  try {
    ...
    const assessments = await db
      ...
      .find({ userId: req.user._id, category: 'tax_profit' })
      .sort({ createdAt: -1 })
      .limit(10)
      .toArray();
