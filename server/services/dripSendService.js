...
class DripSendService {
  ...
  async _runBucket(type, bucket, since) {
    ...
    if (!bucket || !bucket.code) return out;                // bucket not configured
    ...
    if (remaining <= 0) return out;                         // quota already met today

    const codeDoc = await this.promoCodes.findByCode(bucket.code);
    if (!codeDoc) return out;                               // code deleted/renamed — skip safely
    ...
    if (candidates.length === 0) return out;                // list dry — stops cleanly
